...
def search_and_aggregate(category, subcategory, pincode, budget):
    ...
    # Filter potential matches based on pincode
    df_filtered = df_sub[df_sub['Location (Pincode)'] == int(pincode)]
    ...
    user_node = f"user_{pincode}_{budget}"
    G.add_node(user_node, bipartite=0)  # User side

    for idx, row in df_filtered.iterrows():
        option_node = f"option_{idx}"
        G.add_node(option_node, bipartite=1)  # Available options side

        # Define edge weight based on how closely the budget matches
        weight = 1 / (1 + abs(row['Budget'] - int(budget)))  # Higher weight for closer match
        G.add_edge(user_node, option_node, weight=weight)

    # Solve maximum weight bipartite matching
    matching = nx.max_weight_matching(G, maxcardinality=True)

    # Find the best match for the user
    best_match = [pair for pair in matching if user_node in pair]

    if not best_match:
        return None, None

    matched_option = best_match[0][1]  # The matched node from dataset
    matched_index = int(matched_option.split('_')[-1])
    matched_data = df_filtered.loc[matched_index]

    # Compute metrics for the matched data
    metrics = matched_data.select_dtypes(include=['number']).to_dict()
